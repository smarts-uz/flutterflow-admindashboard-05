...
class ResponsiveAppSupabaseUser extends BaseAuthUser {
  ResponsiveAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> responsiveAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ResponsiveAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
